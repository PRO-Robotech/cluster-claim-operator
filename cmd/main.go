...
import (
	...
	"github.com/PRO-Robotech/cluster-claim-operator/internal/controller"
...
func init() {
	...
	utilruntime.Must(clusterclaimv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.BoolVar(&enableWebhook, "enable-webhook", true,
		"Enable the validating webhook. Disable when running without cert-manager.")
	flag.StringVar(&remoteNamespace, "remote-namespace", "",
		"Required. Namespace in remote clusters where ConfigMaps are applied.")
	...
	clusterMgr := remote.NewClusterManager(mgr.GetScheme())
	if err = mgr.Add(clusterMgr); err != nil {
		setupLog.Error(err, "unable to add cluster manager")
		os.Exit(1)
	}
	...
	if err = (&controller.ClusterClaimReconciler{
		...
		Recorder:        mgr.GetEventRecorderFor("clusterclaim-controller"),
	...
	if enableWebhook {
		if err = clusterclaimv1alpha1.SetupClusterClaimWebhookWithManager(mgr); err != nil {
